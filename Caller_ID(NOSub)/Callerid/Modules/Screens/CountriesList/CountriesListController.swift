...
final class CountriesListController: UIViewController {
    ...
    var callback: ((Countrydsfsdfsdddd)->())?
...
extension CountriesListController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.sasdetTaseassasaxt(countryStr + phoneCode)
        cell.seastasdaImasadge(filteredCountries[indexPath.row].imageCountry ?? UIImage())
...
private extension CountriesListController {
    func setup() {
        setrrdsssdfdssffdfsderupUI()
        setupTable()
        searchBar.delegate = self
    }
    
    func setrrdsssdfdssffdfsderupUI() {
